...
> **Note:** This is a *toy* example trained on only 40 examples due to limited compute. To build a production-quality classifier, you would fine-tune on the full SST-2 corpus (~67k examples), use more epochs, larger batches, and (ideally) GPU acceleration.
...
│
├── .ipynb_checkpoints/ # Jupyter autosaves (ignore or remove)
│
├── fine_tune_toy_model.ipynb # Notebook: subsample SST-2 → train DistilBERT → save model
│
├── inference_toy_model.ipynb # Notebook: load & run inference on toy model & official SST-2 checkpoint
│
└── README.md # This file
...
1. **Clone the repo**  
   ...
   cd transformer-experiments/toy-transformer-sentiment

Install dependencies


pip install transformers datasets torch pandas evaluate matplotlib openpyxl
Launch Jupyter
...
inference_toy_model.ipynb: lets you feed in your own sentences and compare predictions from both the toy model and the official SST-2 DistilBERT checkpoint.
...
🤖 Models
Toy model saved under models/distilbert_finetuned/ after running the fine-tuning notebook.

Official SST-2 model (distilbert-base-uncased-finetuned-sst-2-english) is downloaded on demand by the inference notebook.
